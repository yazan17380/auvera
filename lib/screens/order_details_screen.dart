  ...
  void _confirmCancel(Order order) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('Cancel this order?', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('Cancel Order', style: TextStyle(color: AppColors.error, fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    );
  ...
  void _confirmArchive(Order order) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        backgroundColor: AppColors.background,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)),
        title: const Text('Archive this order?', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('Archive', style: TextStyle(color: AppColors.error, fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    );
                  ...
                  if (order.canEdit)
                    IconButton(
                      onPressed: () => Navigator.of(context).push(
                        MaterialPageRoute(builder: (_) => EditOrderScreen(orderId: order.id)),
                      ),
                  ...
                  children: [
                    Row(children: [
                      Container(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: _statusColor(order.status)),
                        ),
                      ),
                    ...
                    ...order.items.map((item) => Container(
                      ...
                      child: Row(children: [
                        ClipRRect(
                          borderRadius: BorderRadius.circular(10),
                          child: Image.network(item.productImageUrl, width: 54, height: 54, fit: BoxFit.cover,
                              errorBuilder: (_, __, ___) => Container(width: 54, height: 54,
                                  color: AppColors.background,
                                  child: const Icon(Icons.image_not_supported_outlined, size: 18, color: AppColors.textHint))),
                        ),
                    ...
                    const Text('Address', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                    const SizedBox(height: 6),
                    Text(order.address, style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),

                    if (order.notes != null && order.notes!.isNotEmpty) ...[
                      const SizedBox(height: 16),
                      const Text('Notes', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                      const SizedBox(height: 6),
                      Text(order.notes!, style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
                    ],

                    const SizedBox(height: 16),
                    const Text('Payment', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                    const SizedBox(height: 6),
